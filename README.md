...
| 管理画面 | 合言葉を知っている人だけ。全投稿の一覧・編集・削除・表示順の並び替え |

いいねの重複は端末単位で防止（`moshimoshi-community/liked`）。
「はじめて画面（スキップ／使い方を聞く）」は**毎回起動時に表示**します。ヘッダーの「使い方」からも開けます。

## 管理者モード

- **入り方**：フィード画面のタイトル「もしもコミュニティ」を**すばやく5回タップ** → 合言葉を入力 → 上部に管理バー＋「投稿の管理」画面
- **管理画面でできること**：全投稿を一覧表示し、各行で「編集」「削除」「↑↓で表示順の並び替え」。種類でフィルタも可。編集は投稿フォームを再利用（種類は変更不可）
- **合言葉の設定**：
